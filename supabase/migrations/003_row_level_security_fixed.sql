...
CREATE POLICY "users_delete_policy" ON users
    ...
    USING (
        company_id = get_current_user_company_id()
        AND get_current_user_role() = 'admin'
        AND id != (SELECT auth.uid()) -- 自分自身は削除不可
    );
...
CREATE POLICY "growing_tasks_select_policy" ON growing_tasks
    ...
    USING (
        EXISTS (
            ...
            AND vegetables.company_id = get_current_user_company_id()
        )
    );
...
CREATE POLICY "growing_tasks_update_policy" ON growing_tasks
    ...
    USING (
        ...
        AND (
            ...
            OR
            (get_current_user_role() = 'operator' AND assigned_to = (SELECT auth.uid()))
        )
    );
...
CREATE POLICY "operation_logs_update_policy" ON operation_logs
    ...
    USING (
        ...
        AND (
            get_current_user_role() IN ('admin', 'manager')
            OR
            (get_current_user_role() = 'operator' AND created_by = (SELECT auth.uid()))
        )
    );
...
CREATE POLICY "notifications_insert_policy" ON notifications
    ...
    WITH CHECK (
        EXISTS (
            ...
            AND target_user.company_id = get_current_user_company_id()
        )
        AND get_current_user_role() IN ('admin', 'manager')
    );
...
DROP TRIGGER IF EXISTS audit_vegetables ON vegetables;
DROP TRIGGER IF EXISTS audit_growing_tasks ON growing_tasks;
