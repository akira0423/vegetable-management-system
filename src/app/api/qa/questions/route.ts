...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        asker:qa_user_profiles!qa_questions_asker_id_fkey(
          display_name,
          avatar_url,
          reputation_score
        )
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 開発環境: 認証されていない場合はテストユーザーを取得または作成
    // 本番環境: 認証必須
    let userId: string;
    ...
    if (!user) {
      ...
      if (testProfile) {
        ...
        // テストユーザーが存在しない場合はエラー
        // （SQLマイグレーションを先に実行する必要がある）
        return NextResponse.json(
          { error: 'Test user not found. Please run the RLS migration first.' },
          { status: 500 }
        );
      }
    } else {
