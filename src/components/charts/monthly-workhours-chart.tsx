...
export default function MonthlyWorkHoursChart({ companyId, selectedVegetable = 'all' }: MonthlyWorkHoursChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    const datasets = workTypes.map(workType => {
      const hoursData = workHoursData.map(d => d.work_types[workType]?.total_hours || 0)
  ...
  }, [workHoursData, weatherDisplayOptions, showCumulativeWorkTime])
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Button
                  ...
                  className={showCumulativeWorkTime ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
                >
                  📊 累積時間線
                </Button>
                ...
                <Button
                  ...
                  className={showComparison ? 'bg-blue-600 hover:bg-blue-700' : ''}
                >
                  📈 前年比較
                </Button>
